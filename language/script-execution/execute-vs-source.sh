...
#
# Terminology
# - subshell: strictly speaking, a subshell is simply a separate shell execution environment that was created as a duplicate of its parent
#   environment. Changes to the subshell environment do not affect the parent environment. Traditionallly, a subshell is created by forking a child
#   process. However, the specification does not require that an implemention must fork a child process to create a subshell environment. Some shells
#   like ksh88 use optimizations to avoid forking when possible because it is an expensive operation. In summary, it is safe to assume that a
#   "subshell" and a forked child process are equivalent in most contexts.
#
# Execution stages
# 
